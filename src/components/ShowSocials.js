...
const ShowSocials = (props) => {
    return (
        <div>
            {props.articles ? props.articles[0] ? <div className="u-margin-bottom">
            ...
            </div> : <JustAnimation/> : null}

            {props.articles ?  props.articles[0] && props.articles[0].title ? <p className="white-text normal-text center-text "><span className="bigger-text red-ish-text capitalize-text">Title:</span> {props.articles[0].title}</p> : <p className="white-text normal-text center-text ">Please enter your title in order to write articles</p> : <JustAnimation/>}
        </div>
    )
}
